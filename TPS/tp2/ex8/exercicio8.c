...
void shellsort(Pokemon *array, int n) {
    ...
    do {
        h /= 3;
        for(int i = 0; i < h; i++){
            insercaoPorWeight(array, n, i, h);
        }
    } while (h != 1);
}

void insercaoPorWeight(Pokemon *array, int n, int j, int h){
...
int main() {
    ...
    shellsort(pokemons,quantidadePokemons);
    long long fim = now();
...
for (int i = 1; i < n; i++) {
    ...
    int j = i - 1;
    while (j >= 0 && pokemons[j].weight == key.weight && strcmp(getName(&pokemons[j]), getName(&key)) > 0) {  
