...
void quickSort(Pokemon *pokemons, int low, int high, int *comparacoes) {
    if (low < high) {
        int pi = partition(pokemons, low, high, comparacoes);
        quickSort(pokemons, low, pi - 1, comparacoes);
        quickSort(pokemons, pi + 1, high, comparacoes);
    }
}

void recursiveSelectionSort(Pokemon *pokemons, int index, int size, int *comparacoes, int *movimentacoes) {
    if (index >= size - 1) return;

    int minIndex = index;
    for (int j = index + 1; j < size; j++) {
        (*comparacoes)++;
        if (atoi(pokemons[j].id) < atoi(pokemons[minIndex].id)) {
            minIndex = j;
        }
    }
    if (minIndex != index) {
        swap(&pokemons[index], &pokemons[minIndex]);
        (*movimentacoes)++;
    }
    recursiveSelectionSort(pokemons, index + 1, size, &comparacoes, &movimentacoes);
}
...
int main() {
    ...
    int i,j;
    for (i = 0; i < quantidadePokemons - 1; i++) {
    for (j = 0; j < quantidadePokemons - 1 - i; j++) { 
        comparacoes++;
        if (atoi(pokemons[j].id) > atoi(pokemons[j + 1].id)) { 
            movimentacoes += 3;
