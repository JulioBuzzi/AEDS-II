...
void quicksortRec(Pokemon *array, int esq, int dir) {
    ...
    while (i <= j) {
        while (array[i].generation < pivo.generation) i++;
        while (array[j].generation > pivo.generation) j--;
        if (i <= j) {
...
int main() {
    ...
    quicksort(pokemons,quantidadePokemons);
     // ORDENAR POR NOME
int n = quantidadePokemons; 
for (int i = 1; i < n; i++) {
    Pokemon key = pokemons[i];
    int j = i - 1;
    while (j >= 0 && pokemons[j].generation == key.generation && strcmp(getName(&pokemons[j]), getName(&key)) > 0) {  
