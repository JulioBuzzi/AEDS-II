...
void inicializarLista(Lista* lista, int tam) 
{
    lista->tam = tam;
    lista->n = 0;
    lista->pokemons = malloc(tam * sizeof(Pokemon));
...
{
    int novoTam = lista->tam * 2;
    lista->pokemons = realloc(lista->pokemons, novoTam * sizeof(Pokemon));
    if (lista->pokemons == NULL) 
    {
        perror("Erro ao redimensionar a lista.");
        exit(1);
    }
    lista->tam = novoTam;
}
